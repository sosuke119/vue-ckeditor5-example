...
class MyUploadAdapter {
  ...
  _initRequest () {
    ...
    xhr.open('POST', '預設的上傳網址', true)
    xhr.responseType = 'json'
  }

  // 初始化 XMLHttpRequest 监听.
  _initListeners (resolve, reject, file) {
    // store.commit('LOADINGSTATUS', true) // 開始上傳後端 - Loading圖啟動
    const xhr = this.xhr
    ...
    xhr.addEventListener('abort', () => {
      reject()
      // store.commit('LOADINGSTATUS', false) // 取消上傳 - Loading圖取消
    })
    xhr.addEventListener('load', () => {
      ...
      // store.commit('LOADINGSTATUS', false) // 成功上傳 - Loading圖取消
    })
  ...
  _sendRequest (file) {
    ...
    data.append('files', file) // 上传的参数data
